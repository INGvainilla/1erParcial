...
import {
  PosService, PosItemInput, PosVentaCreate, PosProductoLookupResponse, PosTicketResponse,
  TicketConsultaResponse, TicketItemLookup, DevolucionCreate, DevolucionTicketResponse
...
@Component({
  ...
  template: `
    <div class="pos-container">
      ...
      <div class="pos-main-grid">
        ...
        <div class="pos-catalog-column glass-panel">
          ...
          <div class="pos-search-box">
            ...
            <!-- Botón para Devolución y Cambio de Prendas (CU25) -->
            <button class="btn-load-dev" (click)="openDevolucionModal()" title="Devolución y Cambio de Prendas (CU25)">
              <i class="fas fa-exchange-alt"></i>
              <span>Devoluciones (CU25)</span>
            </button>
          </div>
      ...
      <!-- MODAL CU25: Gestionar Devolución y Cambio de Prendas -->
      <div *ngIf="showDevolucionModal" class="pos-modal-overlay">
        <div class="pos-modal-box dev-modal-box glass-panel">
          ...
          <div class="modal-body dev-modal-body">
            ...
            <div class="dev-search-bar">
              ...
              <button class="btn-scan" (click)="consultarTicketDevolucion()" [disabled]="isLoadingTicketDev">
                <i class="fas" [class.fa-search]="!isLoadingTicketDev" [class.fa-spinner]="isLoadingTicketDev" [class.fa-spin]="isLoadingTicketDev"></i>
            ...
            <div *ngIf="ticketDevData" class="ticket-info-card">
              ...
              <!-- Si hay prenda seleccionada y el ticket es válido -->
              <div *ngIf="selectedDevItem && ticketDevData.es_valido_14_dias" class="dev-config-panel">
                <!-- Inspección Física -->
                <div class="inspection-box">
                  <label class="config-label"><i class="fas fa-microscope"></i> Inspección Física en Mostrador:</label>
                  <div class="radio-options-row">
                    <label class="radio-pill" [class.active]="devEstadoFisico === 'APTO_VENTA'">
                      <input type="radio" name="estadoFisico" value="APTO_VENTA" [(ngModel)]="devEstadoFisico" />
                      <i class="fas fa-check"></i> Apto para la Venta (Etiquetas intactas)
                    </label>
                    <label class="radio-pill warn" [class.active]="devEstadoFisico === 'DEFECTUOSO_MERMA'">
                      <input type="radio" name="estadoFisico" value="DEFECTUOSO_MERMA" [(ngModel)]="devEstadoFisico" />
                      <i class="fas fa-ban"></i> Defectuoso / Merma (Tara de confección)
                    </label>
                  </div>
                </div>
                ...
                <div class="resolution-box">
                  <label class="config-label"><i class="fas fa-hand-holding-usd"></i> Modalidad de Compensación:</label>
                  <div class="resolution-tabs">
                    <button class="res-tab" [class.active]="devTipoResolucion === 'CAMBIO_VARIANTE'" (click)="devTipoResolucion = 'CAMBIO_VARIANTE'; recalcularDiferencia()">
                      <i class="fas fa-tshirt"></i> Cambio por Talla / Variante
                    </button>
                    <button class="res-tab" [class.active]="devTipoResolucion === 'VALE_CREDITO'" (click)="devTipoResolucion = 'VALE_CREDITO'">
                      <i class="fas fa-gift"></i> Vale de Crédito Tienda
                    </button>
                    <button class="res-tab" [class.active]="devTipoResolucion === 'REEMBOLSO_EFECTIVO'" (click)="devTipoResolucion = 'REEMBOLSO_EFECTIVO'">
                      <i class="fas fa-money-bill-wave"></i> Reembolso Efectivo
                    </button>
                  </div>
                </div>

                <!-- Opciones de Cambio de Variante (Factor Talla) -->
                <div *ngIf="devTipoResolucion === 'CAMBIO_VARIANTE'" class="variant-change-subpanel">
                  <div class="variant-form-row">
                    <div class="form-group">
                      <label>Nueva Talla Solicitada:</label>
                      <select [(ngModel)]="nuevaTallaCambio" (change)="recalcularDiferencia()" class="dev-select">
                        <option value="S">Talla S (-5% descuento de tela)</option>
                        <option value="M">Talla M (Precio Base)</option>
                        <option value="L">Talla L (+5% factor de tela)</option>
                        <option value="XL">Talla XL (+10% factor de tela)</option>
                        <option value="XXL">Talla XXL (+15% factor de tela)</option>
                      </select>
                    </div>
                    <div class="form-group">
                      <label>Color Disponible:</label>
                      <select [(ngModel)]="nuevoColorCambio" class="dev-select">
                        <option [value]="selectedDevItem.color">{{ selectedDevItem.color }} (Mismo Color)</option>
                  ...
                  <div class="difference-calc-box">
                    <div class="calc-row">
                      <span>Valor Prenda Original Devuelta:</span>
                      <strong>Bs. {{ selectedDevItem.precio_unitario | number:'1.2-2' }}</strong>
                    </div>
                    <div class="calc-row">
                      <span>Precio Nueva Variante (Factor {{ getFactorTallaLabel() }}):</span>
                      <strong>Bs. {{ precioNuevaVariante | number:'1.2-2' }}</strong>
                    </div>
                    <div class="calc-row diff-highlight">
                      <span>{{ diferenciaCambio >= 0 ? 'Diferencia a Cobrar al Cliente:' : 'Saldo a Favor del Cliente:' }}</span>
                      <strong [class.text-danger]="diferenciaCambio > 0" [class.text-success]="diferenciaCambio <= 0">
                        Bs. {{ (diferenciaCambio >= 0 ? diferenciaCambio : -diferenciaCambio) | number:'1.2-2' }}
                      </strong>
                    </div>
                    <div class="audit-note">
                      <i class="fas fa-book"></i> Asiento Contable: Reingreso a inventario valorado al CPP histórico inmutable (Bs. {{ selectedDevItem.costo_historico_cpp | number:'1.2-2' }}).
                    </div>
                  </div>
                </div>
                ...
                <div class="form-group motivo-group">
                  ...
                  <select [(ngModel)]="devMotivo" class="dev-select">
                    <option value="Cambio de talla por ajuste ergonómico">Cambio de talla por ajuste ergonómico</option>
                    <option value="Preferencia de color o estilo">Preferencia de color o estilo</option>
                    <option value="Prenda con tara o defecto de confección">Prenda con tara o defecto de confección</option>
                    <option value="Insatisfacción con la prenda">Insatisfacción con la prenda</option>
                  </select>
                </div>
              </div>
            </div>
          ...
          <div class="modal-footer">
            <button class="btn-secondary" (click)="cerrarDevolucionModal()">Cancelar</button>
            <button
              class="btn-primary"
              [disabled]="!selectedDevItem || !ticketDevData?.es_valido_14_dias || isProcessingDev"
              (click)="confirmarDevolucion()"
            >
              <i class="fas" [class.fa-check-double]="!isProcessingDev" [class.fa-spinner]="isProcessingDev" [class.fa-spin]="isProcessingDev"></i>
              <span>{{ isProcessingDev ? 'Procesando en Kardex...' : 'Confirmar Cambio y Emitir Ticket Fiscal (CU25)' }}</span>
            </button>
          </div>
        </div>
      </div>

      <!-- MODAL TICKET TÉRMICO DEVOLUCIÓN (CU25) -->
      <div *ngIf="showDevReceiptModal && ultimoTicketDev" class="pos-modal-overlay">
        <div class="thermal-ticket-dialog glass-panel">
          <div class="dialog-actions-bar no-print">
            <button class="btn-action-print" (click)="printTicket()">
              <i class="fas fa-print"></i> Imprimir Comprobante (80mm)
            </button>
            <button class="btn-action-new" (click)="cerrarDevReceiptModal()">
              <i class="fas fa-check"></i> Finalizar Trámite
            </button>
          </div>
          ...
          <div class="thermal-receipt" id="print-area">
            <div class="receipt-header">
              <div class="receipt-logo">FASHION STORE S.R.L.</div>
              <div class="receipt-line bold">COMPROBANTE FISCAL DE DEVOLUCIÓN</div>
              <div class="receipt-line">SUCURSAL: {{ ultimoTicketDev.sucursal_nombre }}</div>
              <div class="receipt-divider">================================</div>
              <div class="receipt-title">{{ ultimoTicketDev.tipo_resolucion }}</div>
              <div class="receipt-line bold">N° DEVOLUCIÓN: {{ ultimoTicketDev.nro_devolucion }}</div>
              <div class="receipt-line">TICKET ORIGINAL: {{ ultimoTicketDev.nro_ticket_original }}</div>
              <div class="receipt-divider">--------------------------------</div>
            </div>
            ...
            <div class="receipt-info-block">
              <div class="info-row">
                <span>FECHA:</span>
                <span>{{ ultimoTicketDev.fecha_hora | date:'dd/MM/yyyy HH:mm:ss' }}</span>
              </div>
              <div class="info-row">
                <span>CAJERO:</span>
                <span>{{ ultimoTicketDev.cajero_nombre }}</span>
              </div>
              <div class="info-row">
                <span>CLIENTE:</span>
                <span>{{ ultimoTicketDev.cliente_nombre }}</span>
              </div>
              <div class="info-row">
                <span>MOTIVO:</span>
                <span>{{ ultimoTicketDev.motivo }}</span>
              </div>
            </div>
            ...
            <div class="receipt-totals">
              <div class="total-row">
                ...
                <span>Bs. {{ ultimoTicketDev.total_devuelto | number:'1.2-2' }}</span>
              </div>
              <div *ngIf="ultimoTicketDev.item_nuevo_entregado" class="total-row">
                <span>NUEVA PRENDA ({{ ultimoTicketDev.item_nuevo_entregado.talla }}):</span>
                <span>Bs. {{ ultimoTicketDev.item_nuevo_entregado.total_nuevo | number:'1.2-2' }}</span>
              </div>
              <div *ngIf="ultimoTicketDev.diferencia_cobrada !== 0" class="total-row grand">
                <span>DIFERENCIA COBRADA:</span>
                <span>Bs. {{ ultimoTicketDev.diferencia_cobrada | number:'1.2-2' }}</span>
              </div>
              <div *ngIf="ultimoTicketDev.codigo_vale" class="total-row grand" style="color: #047857;">
                <span>CÓDIGO DE VALE:</span>
                <span>{{ ultimoTicketDev.codigo_vale }}</span>
              </div>
            </div>
            ...
            <div class="receipt-footer">
              <p class="legal-text">{{ ultimoTicketDev.mensaje_kardex }}</p>
              <div class="receipt-qr-sim">
                <i class="fas fa-qrcode"></i>
              </div>
              <p class="receipt-code">COMPROBANTE AUTORIZADO - SISTEMA POS V3</p>
...
export class PosTerminalComponent implements OnInit, OnDestroy {
  ...
  selectedDevItem?: TicketItemLookup;
  devEstadoFisico: 'APTO_VENTA' | 'DEFECTUOSO_MERMA' = 'APTO_VENTA';
  devTipoResolucion: 'CAMBIO_VARIANTE' | 'VALE_CREDITO' | 'REEMBOLSO_EFECTIVO' = 'CAMBIO_VARIANTE';
  nuevaTallaCambio: string = 'L';
  nuevoColorCambio: string = '';
  devMotivo: string = 'Cambio de talla por ajuste ergonómico';
  precioNuevaVariante: number = 0;
  diferenciaCambio: number = 0;
  ultimoTicketDev?: DevolucionTicketResponse;
  showDevReceiptModal: boolean = false;
  ...
  // ========================================================================
  // CU25: Métodos de Gestión de Devoluciones y Cambios
  // ========================================================================
  openDevolucionModal() {
  ...
  consultarTicketDevolucion() {
    if (!this.ticketDevSearchTerm.trim()) {
      this.toast.warning('Ticket Requerido', 'Ingrese o escanee el número de ticket o factura.');
      return;
    }

    this.isLoadingTicketDev = true;
    this.posService.consultarTicketDevolucion(this.ticketDevSearchTerm).subscribe({
      next: (resp) => {
        this.isLoadingTicketDev = false;
        this.ticketDevData = resp;
        if (resp.items && resp.items.length > 0) {
          this.selectItemDevolucion(resp.items[0]);
        }
        if (resp.es_valido_14_dias) {
          this.toast.success('Ticket Encontrado', `Venta válida: ${resp.dias_transcurridos} días transcurridos.`);
        } else {
          this.toast.error('Plazo Vencido', resp.mensaje_plazo);
        }
      },
      error: (err) => {
        this.isLoadingTicketDev = false;
        const msg = err.error?.detail || 'Ticket no encontrado o error en servidor.';
        this.toast.error('Búsqueda Fallida', msg);
      }
    });
  }

  selectItemDevolucion(item: TicketItemLookup) {
    this.selectedDevItem = item;
    this.nuevoColorCambio = item.color;
    this.nuevaTallaCambio = item.talla === 'M' ? 'L' : 'M';
    this.recalcularDiferencia();
  }

  getFactorTallaLabel(): string {
    const factor = this.getFactorTalla(this.nuevaTallaCambio);
    const pct = Math.round((factor - 1.0) * 100);
    return pct > 0 ? `+${pct}%` : (pct < 0 ? `${pct}%` : 'Base 0%');
  }

  recalcularDiferencia() {
    if (!this.selectedDevItem) return;
    const factor = this.getFactorTalla(this.nuevaTallaCambio);
    this.precioNuevaVariante = Math.round(this.selectedDevItem.precio_unitario * factor * 100) / 100;
    this.diferenciaCambio = Math.round((this.precioNuevaVariante - this.selectedDevItem.precio_unitario) * 100) / 100;
  }

  confirmarDevolucion() {
    if (!this.selectedDevItem || !this.ticketDevData) {
      this.toast.warning('Datos Incompletos', 'Seleccione la prenda que desea devolver.');
      return;
    }

    if (!this.ticketDevData.es_valido_14_dias) {
      this.toast.error('Operación Bloqueada', 'El plazo de 14 días para devolución ha vencido.');
      return;
    }

    this.isProcessingDev = true;

    const payload: DevolucionCreate = {
      nro_ticket_original: this.ticketDevData.numero_factura,
      motivo: this.devMotivo,
      tipo_resolucion: this.devTipoResolucion,
      items: [
        {
          id_producto: this.selectedDevItem.id_producto,
          talla: this.selectedDevItem.talla,
          color: this.selectedDevItem.color,
          cantidad: 1,
          estado_fisico: this.devEstadoFisico
        }
      ],
      cambio_info: this.devTipoResolucion === 'CAMBIO_VARIANTE' ? {
        nuevo_producto_id: this.selectedDevItem.id_producto,
        nueva_talla: this.nuevaTallaCambio,
        nuevo_color: this.nuevoColorCambio || this.selectedDevItem.color,
        nueva_cantidad: 1
      } : undefined
    };

    this.posService.procesarDevolucion(payload, this.selectedSucursalId).subscribe({
      next: (ticketResp) => {
        this.isProcessingDev = false;
        this.ultimoTicketDev = ticketResp;
        this.showDevolucionModal = false;
        this.showDevReceiptModal = true;
        this.toast.success('Devolución Exitosa', `Comprobante ${ticketResp.nro_devolucion} emitido y Kardex actualizado.`);
        this.loadCatalog();
      },
      error: (err) => {
        this.isProcessingDev = false;
        const msg = err.error?.detail || 'Error al procesar la devolución en caja.';
        this.toast.error('Error Devolución', msg);
      }
    });
  }

  cerrarDevReceiptModal() {
    this.showDevReceiptModal = false;
  }
}
