...
import 'package:fashionstore_mobile/modules/p06_reservas_presenciales/reservas/views/crear_reserva_screen.dart';
import 'package:fashionstore_mobile/modules/p03_catalogo_estilismo_ia/ar_viewer/views/vestidor_ar_screen.dart';
...
class _DetallePrendaScreenState extends State<DetallePrendaScreen> {
  ...
  double get _precioActual {
    if (_selectedTalla.precio != null && _selectedTalla.precio! > 0) {
      return _selectedTalla.precio!;
    }
    const factores = {
    ...
    final factor = _selectedTalla.factor ?? factores[_selectedTalla.talla.trim().toUpperCase()] ?? 1.00;
    return widget.prenda.precioFinal * factor;
  }

  double get _precioBaseActual {
    if (widget.prenda.descuentoPct <= 0) return _precioActual;
    const factores = {
      'S': 0.95, 'M': 1.00, 'L': 1.05, 'XL': 1.10, 'XXL': 1.15,
      '30': 0.95, '32': 1.00, '34': 1.05, '36': 1.10,
      '38': 0.95, '40': 1.00, '42': 1.05, '44': 1.10,
    ...
    final factor = _selectedTalla.factor ?? factores[_selectedTalla.talla.trim().toUpperCase()] ?? 1.00;
    return widget.prenda.precioBase * factor;
  }
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Column(
                            ...
                            children: [
                              Text(
                                'Bs. ${_precioActual.toStringAsFixed(2)}',
                                style: const TextStyle(
                                  fontSize: 22,
                              ...
                              if (p.descuentoPct > 0)
                                Text(
                                  'Bs. ${_precioBaseActual.toStringAsFixed(2)}',
                                  style: const TextStyle(
                                    fontSize: 13,
                      ...
                      if (p.tallas.isNotEmpty) ...[
                        Row(
                          ...
                          children: [
                            ...
                            Text(
                              'Precio por talla: Bs. ${_precioActual.toStringAsFixed(2)}',
                              style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: AppTheme.accentGold),
                            ),
                          ],
                        ),
                        const SizedBox(height: 8),
