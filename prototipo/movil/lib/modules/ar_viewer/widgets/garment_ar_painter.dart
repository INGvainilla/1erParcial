...
class GarmentArPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (garmentImage != null) {
      ...
      if (pose.opacity < 1.0) {
        paint.color = Color.fromRGBO(255, 255, 255, pose.opacity.clamp(0.0, 1.0));
      }

      // Tintado textil respetando relieves y pliegues naturales
      if (tintColor != Colors.transparent && tintColor.alpha > 0) {
        paint.colorFilter = ColorFilter.mode(
          tintColor.withOpacity(0.42 * pose.opacity),
          BlendMode.color,
        );
      }

      // Renderizado directo de la prenda completa sin desgarros de malla
