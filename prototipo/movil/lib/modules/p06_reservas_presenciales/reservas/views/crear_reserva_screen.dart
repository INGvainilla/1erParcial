...
class _CrearReservaScreenState extends State<CrearReservaScreen> {
  ...
  double _calcularPrecioPrenda(PrendaCatalogo p) {
    if (widget.tallaPreseleccionada != null) {
      final t = p.tallas.firstWhere(
        (element) => element.talla.trim().toUpperCase() == widget.tallaPreseleccionada!.trim().toUpperCase(),
        orElse: () => PrendaTalla(idTalla: 0, talla: '', orden: 0),
      );
      ...
      final factor = factores[widget.tallaPreseleccionada!.trim().toUpperCase()] ?? 1.0;
      return p.precioFinal * factor;
    }
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Text(
                      'Bs. ${_calcularPrecioPrenda(p).toStringAsFixed(2)}',
